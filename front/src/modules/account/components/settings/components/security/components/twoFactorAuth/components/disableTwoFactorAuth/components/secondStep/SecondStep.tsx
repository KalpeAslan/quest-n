...
const TwoFactorSecondStep = ({
  ...
  const handleError = (error: any) => {
    ...
    if (response.status === 400 && response.data.errorCode === 1001) {
      formik.setErrors({
        code: t({
          id: "uJ9SNSmULAsFTTLLysE3SK-account",
          message: "Wrong code",
        }),
      });
    ...
    LoggerService.error("Error during delete 2fa - confirm step", error);
  };
  ...
  const validationSchema = yup.object({
    code: yup
      ...
      .min(
        6,
        t({ id: "qo75zmHYwqadEg3HbDPVe9-account", message: "Min length 6" }),
      )
      .required(
        t({
          id: "7TUw4Q1nSXbW4ubZRCFBas-account",
  ...
  return (
    <>
      <Box className="c-font-14-20 c-font-color" component="p">
        <Trans id="5h1QfucNkvcqaFeSWX4Sh7-account">
      ...
      <Box className="c-font-14-20 c-font-color" component="p" mb={1.2}>
        <Trans id="uFsNVX58zSEk2zKcjBUg2g-account">
      ...
      >
        <Box mb={{ xs: 2, md: formik.errors.code ? 2 : 3 }}>
          <Input
            ...
            placeholder={t({
              id: "5Zrc1uTkSKA4LeZxtRXdfc-account",
              message: "Enter code",
            })}
            name="code"
            type="text"
        ...
        <Box mb={2}>
          <Button
            ...
            disabled={isDisabled}
          >
            <Trans id="oN1okoEbLYZz5gcPGx96Fr-account">Confirm</Trans>
          </Button>
        </Box>
      </FormWrapper>
      ...
      <FooterWrapper>
        ...
        >
          <Trans id="5vAuPsoSXoVQwJ9n8qaWae-account">Resend SMS code</Trans>
        </Button> */}
        ...
        >
          <Trans id="4QnAihpu3nSMorGTFGGgcn-account">Cancel</Trans>
        </Button>
      </FooterWrapper>
    </>
  );
