...
import { ReTweetQuoteTwitterLoyaltyTaskBody } from '../../../../../db/types/interfaces/loyalty/tasks';
import { BadRequestError, BadRequestErrorKeys } from '../../../../errors';
import { twitterApiRequests } from '../../../apis/twitterApiRequests';

export const isReTweetQuoteLoyaltyTask = async (investorId: number, loyaltyTask: LoyaltyTask) => {
  ...
  try {
    ...
    const status = userTweets.some(
      (elem) =>
        elem.referenced_tweets[0].type === 'quoted' &&
        elem.referenced_tweets[0].id === (loyaltyTask.body as ReTweetQuoteTwitterLoyaltyTaskBody).tweetId,
    );
